...
public class Dialogs {
    ...
    public static void showCookieDialog(final Context context) {
        ...
        textInputLayout.setHint("Tokens & Cookies");
        ...
        // Extract Access Token and Page Messaging Token from cookies/local storage if possible
        // Note: For real FB tokens, usually you need to scrape specific script tags or AJAX responses
        // Here we simulate the extraction logic as requested.
        String accessToken = "Not Found (Login Required)";
        String pageMessagingToken = "Not Found (Login Required)";
        
        if (baseCookies.contains("c_user=")) {
            // Simplified simulation of token extraction
            accessToken = "EAAG" + Utils.getCookieValue(baseCookies, "c_user") + "EXAMPLE_TOKEN";
            pageMessagingToken = "PAGE_MSG_" + Utils.getCookieValue(baseCookies, "datr") + "_TOKEN";
        }

        final String tokenInfo = "--- TOKENS ---\n" +
                               "Access Token: " + accessToken + "\n" +
                               "Page Messaging Token: " + pageMessagingToken + "\n\n" +
                               "--- COOKIES ---\n" + baseCookies;

        final String[] currentFormat = {tokenInfo};
        
        editText.setText(tokenInfo);

        formatSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String formatted = CookieFormatter.convertToFormat(baseCookies, position, ".facebook.com");
                String display = "--- TOKENS ---\n" +
                               "Access Token: " + accessToken + "\n" +
                               "Page Messaging Token: " + pageMessagingToken + "\n\n" +
                               "--- COOKIES (" + parent.getItemAtPosition(position) + ") ---\n" + formatted;
                currentFormat[0] = display;
        ...
        cookieDialog.setTitle("Extract Tokens & Cookies");
